...
Check the playbooks first. Open `../bstack-mode/SKILL.md`, match the task against its Playbooks section, and route there when one fits. A large migration is usually `../bstack-mode/playbooks/multi-phase-plan.md` or `../bstack-mode/playbooks/orchestrate.md`. One task driven to a predicate is `../bstack-mode/playbooks/autonomous-run.md`. This skill is the fallback for a task none of them matches, not a first choice.

When you do land here, design the playbook before writing any code. The deliverable is a sequence of phases that scales rigor to the task, runs the scientific method, and leaves a trail a human can audit after stepping away. Bias toward rigor. Building the wrong thing costs more than being careful.

Open a todo list whose first item is to read the Principles section of `../bstack-mode/SKILL.md`, then add the phases below.

## Phase A: Frame

Ground first, then commit. Do not start until you can state:
...
Present the framing and tradeoffs once. Reversible work proceeds without waiting (`../bstack-mode/principles/never-block-on-the-human.md`), but a multi-hour run earns one checkpoint here.
...
- For a one-way-door design decision, run the `architect` skill, which runs `arena`. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (`../bstack-mode/principles/laziness-protocol.md`).
- Decide what fans out. Parallelize only across seams, and give each writer `isolation: "worktree"` (`../bstack-mode/principles/separate-before-serializing-shared-state.md`). Do not over-fan.
- Write the phase list down. That list is what the human reviews.

Then add the designed steps to the todo list as concrete items, after the Phase C entry and before Phase D. Run each under the Phase C loop and log a Phase D row as each lands, instead of writing the trail at the end.
...
- Pair delegated work with a judge on a different model (`../bstack-mode/references/models.md`) and audit the delegates' artifacts yourself. If a worker games the gate, reset and harden the brief. If the gate itself is wrong, fix the gate in its own change.
