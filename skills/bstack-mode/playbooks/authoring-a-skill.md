...
4. Test behavior. For a structural rule (a file gets read, a tool is or isn't called, a file is written), add or update a case under `evals/` and run `claude plugin eval . --case <name> --no-publish`. For a subjective change to how agents work, run a blind A/B comparison before promoting it.
